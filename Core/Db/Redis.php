...
// 只有一台 Redis 的应用$redis = new RedisCluster();$redis->connect(array('host'=>'127.0.0.1','port'=>6379));
//$redis->set('id',35);var_dump($redis->get('id'));
       
// 有一台 Master 和 多台Slave 的集群应用$redis = new RedisCluster(true);$redis->connect(array('host'=>'127.0.0.1','port'=>6379), true);
// master$redis->connect(array('host'=>'127.0.0.1','port'=>63791), false);
// slave 1$redis->connect(array('host'=>'127.0.0.1','port'=>63792), false);
// slave 2$redis->set('id',100);for($i=1; $i<=100; ++$i){
// var_dump($redis->get('id')).PHP_EOL;
//}
// phpRedis 扩展的更多高级操作$redis = new RedisCluster();$redis->connect(array('host'=>'127.0.0.1','port'=>6379));
//$ret = $redis->getRedis()->ping();
// phpRedis 原始APIvar_dump($ret);